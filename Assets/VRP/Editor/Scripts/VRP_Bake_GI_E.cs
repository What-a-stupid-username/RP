...
namespace vrp
{
    ...
    public class Bake_GI_Window : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (!instance.baking)
            {
                float probe_density = EditorGUILayout.FloatField("GI probe density", instance.probe_density);
                probe_density = probe_density < 0.1f ? 0.1f : probe_density;
                instance.probe_density = probe_density;
                float keep_percent = EditorGUILayout.FloatField("Probe keep percent", instance.keep_percent);
                keep_percent = keep_percent < 0f ? 0f : keep_percent;
                keep_percent = keep_percent > 1 ? 1 : keep_percent;
                instance.keep_percent = keep_percent;

                if (GUILayout.Button("Bake")) instance.StartBake();
            }

            if (instance.baking)
            {
                EditorGUILayout.FloatField("GI probe density", instance.probe_density);
                EditorGUILayout.FloatField("Probe keep percent", instance.keep_percent);
                if (GUILayout.Button("Stop Bake")) instance.EndBake();
                EditorGUILayout.Space();
                Rect rect = GUILayoutUtility.GetRect(18, 18, "TextField");
                EditorGUI.ProgressBar(rect, instance.bakingPercent,instance.bakingState);
                if (instance.bakingMessage != "") EditorGUILayout.LabelField(instance.bakingMessage);
            }
    ...
    public class VRP_Bake_GI_E
    {
        ...
        public string bakingMessage;
        public float bakingPercent;
        IEnumerator bake_ie;
        ...
        public IEnumerator Baking()
        {
            float lastTime = Time.realtimeSinceStartup;
            bakingState = "Init";
            bakingMessage = "";
            yield return null;
            shg.Init(instance.rt);
            ComputeShader sh_cubemap = Resources.Load<ComputeShader>("Shaders/SH_CubeMap");
            int cubemap2sh = sh_cubemap.FindKernel("CubeMap2SH");
            int CubeMap_id = Shader.PropertyToID("CubeMap");
            ComputeBuffer output = new ComputeBuffer(9, 12);
            ComputeBuffer dirs_buffer = new ComputeBuffer(32 * 32, 12);
            ComputeBuffer debug_buffer = new ComputeBuffer(9, 12);
            dirs_buffer.SetData(dirs);
            Vector3Int[] zero = new Vector3Int[9];
            Vector3Int[] res = new Vector3Int[9];            
            if (sh_cubemap == null)
            {
                bakingMessage = "Error: missing compute shader \"SH_CubeMap\"";
                while (true)
                    yield return null;
            }

            bakingState = "Calculate scene bounding box";
            bakingMessage = "Don't touch anything during this state.";
            yield return null;

            var gos = GetAllSceneObjectsWithInactive();
            List<MeshRenderer> renderers = new List<MeshRenderer>();
            foreach (var go__ in gos)
            {
                var re = go__.GetComponent<MeshRenderer>();
                if (re)
                {
                    renderers.Add(re);
                }
            ...
            Bounds bbox = renderers[0].bounds;
            lastTime = Time.realtimeSinceStartup;
            for (int i = 0; i < renderers.Count; i++)
            {
                bbox.Encapsulate(renderers[i].bounds);
                if (i % 50 == 0)
                {
                    float nowTime = Time.realtimeSinceStartup;
                    if (nowTime - lastTime > 0.016)
                    {
                        lastTime = nowTime;
                        yield return null;
                    }
            ...
            bbox.Expand(new Vector3(2,5,2));

            bakingState = "Generating sample positions";
            bakingMessage = "Don't touch anything during this state.";
            yield return null;

            lastTime = Time.realtimeSinceStartup;
            List<Vector3> sps = new List<Vector3>();
            List<Vector3[]> coefs = new List<Vector3[]>();
            Vector3 pos_ = bbox.min;
            while (true)
            {
                if (pos_.z > bbox.max.z) break;
                while (true)
                {
                    if (pos_.y > bbox.max.y) break;
                    while (true)
                    {
                        if (pos_.x > bbox.max.x) break;
                        sps.Add(pos_);
                        pos_.x += probe_density;
                    }
                    pos_.y += probe_density;
                    pos_.x = bbox.min.x;
                }
                pos_.z += probe_density;
                pos_.x = bbox.min.x;
                pos_.y = bbox.min.y;
                float nowTime = Time.realtimeSinceStartup;
                if (nowTime - lastTime > 0.016)
                {
                    lastTime = nowTime;
                    bakingPercent = (pos_.z - bbox.min.z) / bbox.size.z;
                    yield return null;
                }
            }
                                 
            string state0 = "Generating SH probes  ";
            bakingMessage = "Don't touch anything during this state.";
            yield return null;
            lastTime = Time.realtimeSinceStartup;
            sh_cubemap.SetTexture(cubemap2sh, CubeMap_id, rt);
            sh_cubemap.SetBuffer(cubemap2sh, "output", output);
            sh_cubemap.SetBuffer(cubemap2sh, "dirs", dirs_buffer);
            for (int j = 0; j < 9; j++)
            {
                zero[j] = Vector3Int.zero;
            }
            for (int i = 0; i < sps.Count; i++)
            {
                VRPDebuger.DrawBB(Matrix4x4.identity, bbox.min, bbox.max, Color.yellow, 0.05f);
                DrawAxle(sps[i], 0.016f);

                bakingState = state0 + i.ToString() + "/" + sps.Count.ToString();
                bakingPercent = (float)i / sps.Count;

                shg.transform.position = sps[i];
                shg.Draw();
                output.SetData(zero);
            ...
            {
                tmp_vector3 = sps[i];
                tree.Add(new[] { tmp_vector3.x, tmp_vector3.y, tmp_vector3.z }, i);
            }
            tree.Balance();


            bakingState = "Optimizing probes";
            bakingMessage = "";
            yield return null;
            lastTime = Time.realtimeSinceStartup;

            List<bool> keep = new List<bool>(sps.Count);
            for (int i = 0; i < sps.Count; i++)
                keep.Add(true);
            float min_diff = 0.1f;
            float r = probe_density * 2;
            while (true)
            {
                int remain_count = 0;
                int this_turn_delete_count = 0;
                for (int i = 0; i < sps.Count; i++)
                {
                    if (keep[i] == false) continue;
                    remain_count++;
                    Vector3[] self_sh = coefs[i];
                    tmp_vector3 = sps[i];
                    KdTreeNode<float, int>[] nodes;
                    float rr = r;
                    while (true)
                    {
                        nodes = tree.RadialSearch(new[] { tmp_vector3.x, tmp_vector3.y, tmp_vector3.z }, rr);
                        if (nodes.Count() == 1) rr *= 2;
                        else break;
                        if (rr > r * 32) goto next_loop;
                    }
                    float diff = 0;
                    for (int j = 0; j < nodes.Count(); j++)
                    {
                        var nei = nodes[j];
                        if (nei.Value == i) continue;
                        Vector3[] sh = coefs[nei.Value];
                        for (int k = 0; k < 9; k++)
                        {
                            diff += Vector2.Distance(self_sh[k], sh[k]) * coef_compare_weight[k];
                        }
                    }
                    if (diff / (nodes.Count()-1) < min_diff)
                    {
                        remain_count--;
                        this_turn_delete_count++;
                        keep[i] = false;
                        tree.RemoveAt(new[] { tmp_vector3.x, tmp_vector3.y, tmp_vector3.z });
                    }
                    else
                    {
                        DrawAxle(tmp_vector3, 0.5f);
                    }
                    next_loop:;
                }
                float nowTime = Time.realtimeSinceStartup;
                ...
                bakingPercent = (float)(sps.Count - remain_count) / sps.Count / (1 - keep_percent);
                Debug.Log(remain_count);
                Debug.Log(min_diff);
                if (remain_count < sps.Count * keep_percent) break;
                if (this_turn_delete_count == 0)
                {
                    min_diff += 1f;
                }
            }

            bakingState = "Saving result";
            bakingMessage = "Don't touch anything during this state.";
            yield return null;
            go.name = "GI SH";
            go.tag = "GI";
            ...
            for (int i = 0; i < sps.Count; i++)
            {
                if (keep[i])
                {
                    shAsset.poss.Add(sps[i]);
                    SHAsset.SH_Coef coef = new SHAsset.SH_Coef();
                    coef.c = new List<Vector3>(coefs[i]);
                    shAsset.SHs.Add(coef);
                }
            }
            AssetDatabase.CreateAsset(shAsset, "Assets/GI_SH_" +SceneManager.GetActiveScene().name + ".asset");
            SceneView.lastActiveSceneView.Repaint();

            bakingState = "Release resources";
            bakingMessage = "";
            yield return null;
            bool suc_rel = true;
            try {
                var go_ = shg.gameObject;
                GameObject.DestroyImmediate(shg);
                GameObject.DestroyImmediate(go_.GetComponent<Camera>());
                go = null;
            ...
            if (!suc_rel)
            {
                bakingMessage = "Error: Unkown error when release resources";
                while (true)
                    yield return null;
            }
            yield return null;

            bakingState = "Saving results";
            //Todo:
        }

        public void UpdateBake()
        {
            bool next = bake_ie.MoveNext();
            if (!next) EndBake();
        }
        ...
        static void DrawAxle(Vector3 pos, float time = 10)
        {
            Debug.DrawLine(pos, pos + Vector3.right * axle_l, Color.red, time);
            Debug.DrawLine(pos, pos + Vector3.up * axle_l, Color.green, time);
            Debug.DrawLine(pos, pos + Vector3.forward * axle_l, Color.blue, time);
        }

        static float[] coef_compare_weight = { 15, 5, 5, 5, 3, 3, 3, 3, 3 };
