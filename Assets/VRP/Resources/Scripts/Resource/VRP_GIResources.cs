...
namespace vrp
{
    ...
    public class VolumeProbeSampler
    {
        int volume_size_ = -1;
        int volume_size_2 = -1;
        int volume_size_3 = -1;
        float one_div_sqrt_2;
        public float frustum_importance = 0.6f;

        int m_sample_num = -1;

        public int volume_size = 8;

        public int sample_num = 20;

        Vector3Int[] sample_vector;
        Vector3Int[] offset_vectors;
        int[] oct_index;
        public Vector3Int[] samples;
        int[,,] pickupsamples;

        public VolumeProbeSampler()
        {
            one_div_sqrt_2 = 1.0f / Mathf.Sqrt(2.0f);
            offset_vectors = new Vector3Int[8];
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++)
                        offset_vectors[i * 4 + j * 2 + k] = new Vector3Int(i, j, k);

            pickupsamples = new int[6, 2, 4];

            //look up table
            {
                pickupsamples[0, 0, 0] = 2;
                pickupsamples[0, 0, 1] = 3;
                pickupsamples[0, 0, 2] = 6;
                pickupsamples[0, 0, 3] = 7;
                pickupsamples[0, 1, 0] = 0;
                pickupsamples[0, 1, 1] = 1;
                pickupsamples[0, 1, 2] = 4;
                pickupsamples[0, 1, 3] = 5;

                pickupsamples[1, 1, 0] = 2;
                pickupsamples[1, 1, 1] = 3;
                pickupsamples[1, 1, 2] = 6;
                pickupsamples[1, 1, 3] = 7;
                pickupsamples[1, 0, 0] = 0;
                pickupsamples[1, 0, 1] = 1;
                pickupsamples[1, 0, 2] = 4;
                pickupsamples[1, 0, 3] = 5;

                pickupsamples[2, 0, 0] = 4;
                pickupsamples[2, 0, 1] = 5;
                pickupsamples[2, 0, 2] = 6;
                pickupsamples[2, 0, 3] = 7;
                pickupsamples[2, 1, 0] = 0;
                pickupsamples[2, 1, 1] = 1;
                pickupsamples[2, 1, 2] = 2;
                pickupsamples[2, 1, 3] = 3;

                pickupsamples[3, 1, 0] = 4;
                pickupsamples[3, 1, 1] = 5;
                pickupsamples[3, 1, 2] = 6;
                pickupsamples[3, 1, 3] = 7;
                pickupsamples[3, 0, 0] = 0;
                pickupsamples[3, 0, 1] = 1;
                pickupsamples[3, 0, 2] = 2;
                pickupsamples[3, 0, 3] = 3;

                pickupsamples[4, 0, 0] = 1;
                pickupsamples[4, 0, 1] = 3;
                pickupsamples[4, 0, 2] = 5;
                pickupsamples[4, 0, 3] = 7;
                pickupsamples[4, 1, 0] = 0;
                pickupsamples[4, 1, 1] = 2;
                pickupsamples[4, 1, 2] = 4;
                pickupsamples[4, 1, 3] = 6;

                pickupsamples[5, 1, 0] = 1;
                pickupsamples[5, 1, 1] = 3;
                pickupsamples[5, 1, 2] = 5;
                pickupsamples[5, 1, 3] = 7;
                pickupsamples[5, 0, 0] = 0;
                pickupsamples[5, 0, 1] = 2;
                pickupsamples[5, 0, 2] = 4;
                pickupsamples[5, 0, 3] = 6;
            }
        }

        void RecalculatePrerandom()
        {
            volume_size_2 = volume_size_ * volume_size_;
            volume_size_3 = volume_size_2 * volume_size_;

            oct_index = new int[8];
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 2; k++)
                        oct_index[i * 4 + j * 2 + k] = UnityEngine.Random.Range(0, volume_size_3);

            sample_vector = new Vector3Int[volume_size_3];
            for (int i = 0; i < volume_size_; i++)
                for (int j = 0; j < volume_size_; j++)
                    for (int k = 0; k < volume_size_; k++)
                        sample_vector[i * volume_size_2 + j * volume_size_ + k] = new Vector3Int(i, j, k);

            for (int i = 0; i < volume_size_3; i++)
            {
                int peer_index = UnityEngine.Random.Range(i, volume_size_3);
                var temp = sample_vector[peer_index];
                sample_vector[peer_index] = sample_vector[i];
                sample_vector[i] = temp;
            }

            samples = new Vector3Int[sample_num];
        }

        // Update is called once per frame
        public void Sample(Vector3 forward)
        {
            if (volume_size != volume_size_ || sample_num != m_sample_num)
            {
                m_sample_num = sample_num;
                volume_size_ = volume_size;
                RecalculatePrerandom();
            }

            PickupSamples(forward);
        }

        void PickupSamples(Vector3 foward)
        {
            Vector3 abs_fo = new Vector3(Mathf.Abs(foward.x), Mathf.Abs(foward.y), Mathf.Abs(foward.z));
            int pickup_blocks;

            int inverse = UnityEngine.Random.Range(0f, 1f) > frustum_importance ? 1 : 0;

            if (foward.y > one_div_sqrt_2) pickup_blocks = 0;
            else if (foward.y < -one_div_sqrt_2) pickup_blocks = 1;
            else if (foward.x > abs_fo.z) pickup_blocks = 2;
            else if (foward.x < -abs_fo.z) pickup_blocks = 3;
            else if (foward.z > abs_fo.x) pickup_blocks = 4;
            else pickup_blocks = 5;

            int[] temp_index = new int[4];
            for (int i = 0; i < 4; i++)
                temp_index[i] = oct_index[pickupsamples[pickup_blocks, inverse, i]];
            for (int i = 0; i < sample_num / 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    int index = temp_index[j] + 1;
                    index %= volume_size_3;
                    temp_index[j] = index;
                    samples[i * 4 + j] = sample_vector[index] + offset_vectors[pickupsamples[pickup_blocks, inverse, j]] * volume_size;
                }
            }
            for (int i = 0; i < 4; i++)
                oct_index[pickupsamples[pickup_blocks, inverse, i]] = temp_index[i];
        }
        ...
        //    {
        //        Gizmos.DrawSphere(samples[i], 0.2f);
        //    }
        //}
    }
    ...
    public class GIResources
    {
        ...
        int kernel_Fill;
        int kernel_Denoise;
        int kernel_Realtime_Batched_CubeMap2SH;
        int kernel_Update_Probe_In_Volume;
        ...
        VComputeBuffer pre_random_dirs_buffer;
        VComputeBuffer sampleInfos_buffer;
        VComputeBuffer sh_buffer;

        VolumeProbeSampler volumeProbeSampler;
        ...
        private class ShaderPropertyID
        {
            ...
            public ShaderPropertyID()
            {
                ...
                cubeMapArray = Shader.PropertyToID("CubeMapArray");
                dirs = Shader.PropertyToID("dirs");
                output = Shader.PropertyToID("output");
                sampleInfo = Shader.PropertyToID("sampleInfo");                
            }
        };
...
//#endif
        ...
        { 
            ...
            volumeProbeSampler.sample_num = sampleNum;
            volumeProbeSampler.volume_size = half_volumeSize;

            cubeTexArray.TestNeedModify(32, 32, sampleNum);
            sampleInfos_buffer.TestNeedModify(sampleNum);
            ...
            {
                sampleInfos_[index].index = probe;
                RenderToCube(ref renderContext, cb, camera, ref cullResults, ori + deltaDistance * (Vector3)probe, index++);
            }
            sampleInfos_buffer.data.SetData(sampleInfos_);

            cb.SetComputeTextureParam(m_cs_UpdateSHVolume, kernel_Realtime_Batched_CubeMap2SH, shaderPropertyID.cubeMapArray, cubeTexArray.data);
            cb.SetComputeBufferParam(m_cs_UpdateSHVolume, kernel_Realtime_Batched_CubeMap2SH, shaderPropertyID.dirs, pre_random_dirs_buffer.data);
            cb.SetComputeBufferParam(m_cs_UpdateSHVolume, kernel_Realtime_Batched_CubeMap2SH, shaderPropertyID.output, sh_buffer.data);
            cb.DispatchCompute(m_cs_UpdateSHVolume, kernel_Realtime_Batched_CubeMap2SH, sampleNum, 1, 1);
            
            cb.SetComputeBufferParam(m_cs_UpdateSHVolume, kernel_Update_Probe_In_Volume, shaderPropertyID.output, sh_buffer.data);
        ...
        public GIResources(VRPAsset asset)
        {
            ...
            kernel_Denoise = m_cs_UpdateSHVolume.FindKernel("Denoise");
            ...
            cubeTexArray = new VRenderTextureCubeArray("realtime GI cube texture array", RenderTextureFormat.ARGBFloat);

            pre_random_dirs_buffer = new VComputeBuffer(Marshal.SizeOf(typeof(Vector3)));
            pre_random_dirs_buffer.TestNeedModify(pre_random_dirs.Length);
            pre_random_dirs_buffer.data.SetData(pre_random_dirs);
            sampleInfos_buffer = new VComputeBuffer(Marshal.SizeOf(typeof(SampleInfo)));
            sh_buffer = new VComputeBuffer(Marshal.SizeOf(typeof(Vector3Int)));
