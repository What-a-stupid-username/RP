...
float3 GI_Diffuse(float3 worldPos, float3 normal) {
	...
	float4 encode_sh0 = _GIVolume_0.SampleLevel(sampler_GIVolume_0, gi_position, 4);
	float4 encode_sh1 = _GIVolume_1.SampleLevel(sampler_GIVolume_1, gi_position, 4);

	res += sh9.c[0] * encode_sh0.xyz;
	res += sh9.c[1] * float3(encode_sh0.w, encode_sh1.xy);

	encode_sh0 = _GIVolume_2.SampleLevel(sampler_GIVolume_2, gi_position, 4);

	res += sh9.c[2] * float3(encode_sh1.zw, encode_sh0.x);

	encode_sh1 = _GIVolume_3.SampleLevel(sampler_GIVolume_3, gi_position, 4);

	res += sh9.c[3] * encode_sh0.yzw;

	res += sh9.c[4] * encode_sh1.xyz;

	encode_sh0 = _GIVolume_4.SampleLevel(sampler_GIVolume_4, gi_position, 4);

	res += sh9.c[5] * float3(encode_sh1.w, encode_sh0.xy);

	encode_sh1 = _GIVolume_5.SampleLevel(sampler_GIVolume_5, gi_position, 4);

	res += sh9.c[6] * float3(encode_sh0.zw, encode_sh1.x);

	encode_sh0 = _GIVolume_6.SampleLevel(sampler_GIVolume_6, gi_position, 4);
