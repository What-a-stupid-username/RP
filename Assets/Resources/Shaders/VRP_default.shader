		...
		SubShader
		{
			...
			Pass {
				...
				float4 frag(v2f i) : SV_TARGET {
					float3 color = 0;
					float3 normal = normalize(i.normal);
					float3 tangent = normalize(cross(i.normal,float3(0,1,1)));
					...
					for (int it = 0; it < _LightSum; it++) {
						...
						if (light.pos_type.w == 0) {
							color += SampleLight_Dir(cascade, light, worldPos, normal, tangent, viewDir);
						}
						else if (light.pos_type.w == 1) {
							color += SampleLight_Point(light, worldPos, normal, tangent, viewDir);
						}
